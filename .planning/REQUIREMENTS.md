...
- [x] **BASE-01**: app.json 配置页面路由、窗口样式、自定义tabBar
- [x] **BASE-02**: app.wxss 全局样式重置、CSS变量（颜色/字体）
- [x] **BASE-03**: services/api.js 封装所有接口，预留真实域名，Promise化
- [x] **BASE-04**: 自定义TabBar组件（5栏纯文字，选中黑色，未选中#999999）
- [x] **BASE-05**: 自定义NavBar组件（Home图标+标题+胶囊按钮）
...
- [x] **IDX-07**: 从API获取首页数据（bannerList/productList），含loading/error/empty三态

### 破价专区 (pages/special)

- [x] **SPE-01**: NavBar返回箭头（左）+胶囊（右），"破价专区"为圆角胶囊badge标题
- [x] **SPE-02**: 顶部模特banner（"all products"小字+"所有产品"大字+"点击进入"按钮）
- [x] **SPE-03**: 单列商品卡片（圆角12rpx，阴影，左图右文布局）
- [x] **SPE-04**: 已抢光商品显示黑色半透明遮罩+"已抢光"文字
- [x] **SPE-05**: 商品标签组（不退款/不换款/只换尺码，灰色边框圆角）
- [x] **SPE-06**: 库存文字（"仅剩X件"，#999999）+ 价格 + 圆形加购按钮
- [x] **SPE-07**: 悬浮礼物按钮（同首页）
- [x] **SPE-08**: 从API获取破价商品列表，含三态处理

### 购物车 (pages/cart)

- [x] **CART-01**: NavBar（Home图标+购物车文字左对齐，胶囊右）
- [x] **CART-02**: 页面背景#F5F5F5（浅灰）
- [x] **CART-03**: 空状态：占位插画（200×200rpx）+提示文字+继续逛逛按钮（黑色圆角）
- [x] **CART-04**: 有数据时显示购物车列表（数据从API获取）
- [x] **CART-05**: 从API获取购物车数据，空数组显示空状态
...
- [x] **NEW-06**: 从API获取新品列表，含三态处理

### 会员中心 (pages/mine)

- [x] **MINE-01**: NavBar（Home图标左，"会员中心"居中，胶囊右）
- [x] **MINE-02**: 用户信息区：左侧ID"21>"（粗体）+"V1 普通会员"badge（米色#F5F0E8背景，棕色#8B7355文字）
- [x] **MINE-03**: 右侧圆形头像（直径120rpx，占位符）
...
| IDX-01~07 | Phase 2 | Complete |
| SPE-01~08 | Phase 3 | Complete |
| CART-01~05 | Phase 3 | Complete |
